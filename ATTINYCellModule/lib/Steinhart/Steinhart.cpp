...
uint16_t Steinhart::ThermistorToCelciusFloat(uint16_t BCOEFFICIENT, uint16_t RawADC, float ADCScaleMax) {
#define NOMINAL_TEMPERATURE 25
 if (RawADC>0){
    float steinhart = (ADCScaleMax/(float)RawADC - 1.0);
    steinhart = log(steinhart); // ln(R/Ro)
    ...
    if(steinhart < 0){//sets the bms 1 to represent negative number
      steinhart = -steinhart;
      return 0x8000 | (uint16_t)(steinhart*10);
    }
    return (uint16_t)(steinhart*10);
 }
 return (int16_t)-999;
}
